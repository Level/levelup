...
buster.testCase('batch()', {
  ...
  'chained batch() arguments': {
    ...
    'test batch#put() with missing `key`': function () {
      // key = undefined
      assert.exception(this.batch.put.bind(this.batch, undefined, 'foo1'), err => {
      ...
      // key = null
      assert.exception(this.batch.put.bind(this.batch, null, 'foo1'), err => {
        if (err.name !== 'WriteError') { return false }
        if (err.message !== 'key cannot be `null` or `undefined`') { return false }
        return true
      })
    ...
    'test batch#put() with missing `key` and `value`': function () {
      // undefined
      assert.exception(this.batch.put.bind(this.batch), err => {
        if (err.name !== 'WriteError') { return false }
        if (err.message !== 'key cannot be `null` or `undefined`') { return false }
        return true
      })
      ...
      // null
      assert.exception(this.batch.put.bind(this.batch, null, null), err => {
        if (err.name !== 'WriteError') { return false }
        if (err.message !== 'key cannot be `null` or `undefined`') { return false }
        return true
      })
    ...
    'test batch#del() with missing `key`': function () {
      // key = undefined
      assert.exception(this.batch.del.bind(this.batch, undefined, 'foo1'), err => {
        if (err.name !== 'WriteError') { return false }
        if (err.message !== 'key cannot be `null` or `undefined`') { return false }
        return true
      })
      ...
      // key = null
      assert.exception(this.batch.del.bind(this.batch, null, 'foo1'), err => {
        if (err.name !== 'WriteError') { return false }
        if (err.message !== 'key cannot be `null` or `undefined`') { return false }
        return true
      })
