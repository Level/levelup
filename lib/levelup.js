...
var leveldown    = require('leveldown')
  ...
  , createLevelUP = function (location, options, callback) {
      ...
      var status = 'new'
        ...
        , _recursive_create = function (p, mode, made) { // losely taken from substack/node-mkdirp
            void 0 === mode && (mode = 511 & ~process.umask());
            made || (made = null);

            typeof mode === 'string' && (mode = parseInt(mode, 8));
            p = path.resolve(p);
            ...
            try {
              ...
              switch (err0.code) {
              case 'ENOENT':
                made = _recursive_create(path.dirname(p), mode, made);
                _recursive_create(p, mode, made);
                break;
      ...
      _path = location.split('/').filter(function (_path) {
        return _path != ''
      });
      ...
      if ( !options.createIfPathInexistent ) {
        if ( !fs.existsSync(_path.join('/')) && !fs.existsSync(_path.slice(0, _path.length-1).join('/')) ) {
          error = new errors.InitializationError(
            'Error: ' + _path.join('/') + ' does not exist.')
          if (callback)
            return callback(error)
          throw error
        }
        ...
        if ( !fs.existsSync(_path) )
          try {
            _recursive_create(location);
          } catch (e) {
            error = new errors.InitializationError(
              'Error: ' + _path.join('/') + ' does not exist and failed to be recursively created.')
            if (callback)
              return callback(error)
            throw error
          }
