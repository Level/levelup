...
LevelUP.prototype = {
  ...
  , put: function (key, value, options_, callback_) {
      var callback, options, err, hook, arr

      hook = this._hooks.put

      if (hook) {
        arr = [{ type: "put", key: key, value: value }]
        hook.forEach(function (hook) {
          hook(value, key, arr)
        })
        return this.batch(arr, options_, callback_)
      }
  ...
  , del: function (key, options_, callback_) {
      ...
      hook = this._hooks.del

      if (hook) {
        arr = [{ type: "del", key: key }]
        hook.forEach(function (hook) {
          hook(key, arr)
        })
        return this.batch(arr, options_, callback_)
  ...
  , hook: function (operation, listener) {
    var hooks, hook

    hooks = this._hooks
    hook = hooks[operation]

    if (!hook) {
      hooks[operation] = [listener]
    } else {
      hook.push(listener)
    }
  }

  , removeHook: function (operation, listener) {
    var hook, index

    hook = this._hooks[operation]
    index = hook.indexOf(listener)

    if (index === -1) {
      throw new Error("WTF YOU THINK YOU DOING.")
    }
