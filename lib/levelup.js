...
function LevelUP (location, options, callback) {
  ...
  this.open(callback)
  var self = this
  self.on('put', function (key, value) {
    if (this._piped) self.push({id:key, value:value})
  })
  self.on('del', function (key) {
    if (this._piped) self.push({id:key, _deleted:true})
  })
}

inherits(LevelUP, Transform)

LevelUP.prototype._transform = function (chunk, encoding, callback) {
  if (typeof chunk !== 'object') return callback() // should be in object mode
  if (!chunk.id || (!chunk.value && !chunk._deleted)) return callback() // not conforming to standard
  if (chunk._deleted) return this.del(chunk.id, callback)
  this.put(chunk.id, chunk.value, callback)
}

LevelUP.prototype.pipe = function () {
  this._piped = true
  Transform.prototype.pipe.apply(this, arguments)
}
