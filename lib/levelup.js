...
LevelUP.prototype.open = function (callback) {
  ...
  db.open(this.options, function (err) {
    if (err) {
      return dispatchError(self, new errors.OpenError(err), callback)
    } else {
      self.db.setDb(db)
      self.db = db
      self._status = 'open'
...
LevelUP.prototype.get = function (key_, options, callback) {
  ...
  if (typeof callback != 'function') {
    return dispatchError(
        this
      , new errors.ReadError('get() requires key and callback arguments')
    )
  }
  ...
  if (!this._isOpening() && !this.isOpen()) {
    return dispatchError(
        this
      , new errors.ReadError('Database is not open')
      , callback
    )
  }
...
LevelUP.prototype.put = function (key_, value_, options, callback) {
  ...
  if (key_ === null || key_ === undefined
    ...
    return dispatchError(
        this
       , new errors.WriteError('put() requires key and value arguments')
       , callback
    )
  }
  ...
  if (!this._isOpening() && !this.isOpen()) {
    return dispatchError(
        this
      , new errors.WriteError('Database is not open')
      , callback
    )
  }
  ...
  this.db.put(key, value, options, function (err) {
    if (err) {
      return dispatchError(self, new errors.WriteError(err), callback)
    } else {
...
LevelUP.prototype.del = function (key_, options, callback) {
  ...
  if (key_ === null || key_ === undefined) {
    return dispatchError(
        this
      , new errors.WriteError('del() requires a key argument')
      , callback
    )
  }
...
LevelUP.prototype.batch = function (arr_, options, callback) {
  ...
  if (!Array.isArray(arr_)) {
    return dispatchError(
        this
      , new errors.WriteError('batch() requires an array argument')
      , callback
    )
  }
...
LevelUP.prototype.approximateSize = function (start_, end_, callback) {
  ...
  if (start_ === null || start_ === undefined
    ...
    return dispatchError(
        this
      , new errors.ReadError('approximateSize() requires start, end and callback arguments')
      , callback
    )
  }
