...
function ReadStream (options, db, iteratorFactory) {
  ...
  // Use "le" (less-than or equal-to) and "ge" (greater-than or equal-to) for a more
  // intuitive start/end boundary when reversing a list.
  if (typeof options.le != 'undefined') {
    if (options.reverse) {
      if (typeof options.start != 'undefined')
        throw new Error('Conflicting options "start" and "le" in reversed stream')
      options.start = options.le
    }
    else {
      if (typeof options.end != 'undefined')
        throw new Error('Conflicting options "end" and "le" in stream')
      options.end = options.le
    }
  }
  if (typeof options.ge != 'undefined') {
    if (options.reverse) {
      if (typeof options.end != 'undefined')
        throw new Error('Conflicting options "end" and "ge" in reversed stream')
      options.end = options.ge
    } else {
      if (typeof options.start != 'undefined')
        throw new Error('Conflicting options "start" and "ge" in stream')
      options.start = options.ge
    }
  }
